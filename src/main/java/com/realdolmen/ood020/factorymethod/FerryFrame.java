...
public class FerryFrame extends JFrame implements ActionListener {
	...
	private VehicleCreator vc = new VehicleCreatorImp(); // Creeer een
	...
	public void actionPerformed(ActionEvent event) {
		if (event.getSource() == newVehicle) {
			...
			// Hier gaan we een deel uit de onderstaande commented code kopieren en herschrijven voor de createVehicle

			berths = Integer.parseInt(berthsChoice.getSelectedItem());
			v = vc.createVehicle(checkboxes[0].isSelected(), noPass,reg,berths ); // wordt uit onderstaande commented cpde
			// gehaald en herschreven voor de createVehicle


			// Onderstaande code wordt in comments gezet en aangepast om te werken met de createVehicle
			...
			added = theFerry.addVehicle(v);  // wordt uit bovenstaande code gehaald
